...
<Warning>
Every response an endpoint documents has to be constructed with a literal `new` **inside `__invoke()` itself**.

A response built anywhere else, in a private helper, in a service, in a factory, is invisible to the scanner. The endpoint keeps answering with it at runtime, but the generated document no longer mentions it, and nothing warns you.
</Warning>
...
```php
// A helper builds it. The scanner sees a method call, not a `new`.
return $this->buildPetResponse($pet);

// Assigned from a call rather than from `new`. There is no `new` to trace the variable back to.
$response = $this->petService->list($filters);
return $response;

// A variable only counts as the whole returned expression, never as an argument.
return $this->decorate($response);
```

The first two are the ones that bite in practice, and they usually hit the success response, because that is the one worth building somewhere reusable. The endpoint is then published with no `2xx` at all, carrying only the error responses [derived from its route](/documentation/openapi-generator#automated-error-responses).
...
```php
$page = $this->petService->list($filters);   // returns a page of data, not a response

$response = new ListPetsResponse($page->getPets());
$response->setPaginationResponse($page->getPagination());
...
<Tip>
`ControllerResponseScanner::scan()` returns exactly what the generator will document, so a controller test can pin it:

```php
$this->assertContains(
    GetPetResponse::class,
    ControllerResponseScanner::scan(GetPetController::class)
);
```

Worth doing for every endpoint whose success response is not a plain `return new …`.
</Tip>
