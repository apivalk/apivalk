...
* **PHP 7.4+**: We maintain compatibility with PHP 7.4 while using modern features where possible (like `declare(strict_types=1)` and return type hints).
...
* **PHPStan Level 6**: We use PHPStan for static analysis to ensure code quality and type-safety. All core code must pass level 6 analysis.
