...
`ResourceRequest` in the `__invoke()` signature is what makes it the request class, and the responses your `__invoke()` returns are the ones that end up in the spec. Neither is declared anywhere else.
...
It does not provide a request or response class. `ResourceRequest` is the request class because your `__invoke()` names it, and the responses in the spec are the ones your `__invoke()` returns, plus the error responses the route itself can produce. See [OpenAPI Generator](/documentation/openapi-generator).
