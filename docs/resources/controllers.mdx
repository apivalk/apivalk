...
| Base class | HTTP | Success response |
|---|---|---|
...
The base class provides `getEmptyResource()`. Create and Update also expose a `getResource()` helper — see below.

`ResourceRequest` in the `__invoke()` signature is what makes it the request class, and the responses your `__invoke()` constructs with `new` are the ones that end up in the spec. Neither is declared anywhere else, and a response built outside the method is [not documented](/http/controller#documented-responses).
...
Both outcomes are documented because both are returned: the scan of `__invoke()` finds `ResourceUpdatedResponse` and `ResourceCreatedResponse` and the spec gets a `200` and a `201`.
...
It does not provide a request or response class. `ResourceRequest` is the request class because your `__invoke()` names it, and the responses in the spec are the ones your `__invoke()` [constructs with `new`](/http/controller#documented-responses), plus the error responses the route itself can produce. See [OpenAPI Generator](/documentation/openapi-generator).
