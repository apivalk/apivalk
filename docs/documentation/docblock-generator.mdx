...
2. **Extraction**: Reads the Request class from the controller's `__invoke()` parameter and calls `getRoute()` to find its properties.
3. **Shape Generation**: Creates "Shape" classes (e.g., `GetUserBodyShape`, `GetUserSortingShape`, `GetUserFilteringShape`) in a `Shape/` subdirectory. These classes contain public properties representing your documentation and route-defined sorting/filtering.
   ...
   Filtering gets one extra interface per field (`GetUserFilterStatusShape`), listing only the operators that field declares, so `$request->filtering()->status->` offers `equal` but not `like` when the route only allows `EQ`. The filtering shape names those field shapes and carries the `FilterBag` API itself, which is why its `@method` entry has no `FilterBag|` union: a union would resolve every field through `FilterBag::__get()` as well and report `FilterInterface|null` next to the field shape.
 ...
 * @method Shape\GetUserFilteringShape filtering()
