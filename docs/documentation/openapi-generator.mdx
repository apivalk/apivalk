...
By default, each filter field becomes its own `deepObject` parameter whose properties are the operators that field declares. Clients send `?status[eq]=active&amount[gte]=10`.
...
If you prefer each filter to appear as a flat query parameter without operators (`?status=active`, which resolves to the first declared operator), pass `true` for the `$flatFilters` parameter:
...
- One parameter per field, with its allowed operators as sub-inputs
- Clients send `?status[eq]=active&amount[gte]=10`

With `flatFilters: true`:
- One flat input per field, no operator choice documented
