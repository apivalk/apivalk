...
2. **Metadata Extraction**: For each route, it takes the request class from the controller's `__invoke()` parameter and the response classes from what its body returns, then calls `getDocumentation()` on those classes to obtain the declared `ApivalkRequestDocumentation` / `ApivalkResponseDocumentation`. Route-level metadata (filters, sortings, pagination, rate limit, authorization) is merged in from the `Route` object itself. See [Controller](/http/controller) for how both are derived, and for the two cases that abort generation with a `LogicException`.
