...
* **Documentation Exclusion**: A flag set via `excludeFromDocumentation()` that keeps the route out of the generated OpenAPI spec.
...
public static function getRoute(): Route
{
    return Route::get('/v1/users')
                  ->description('List users')
                  ->filtering([
                      new EnumFilter(new EnumProperty('status', 'Status', ['active', 'inactive']), Operator::EQ),
                      new StringFilter(new StringProperty('type', 'Type'), Operator::IN),
                      new DateFilter(new DateProperty('created_at', 'Creation date'), Operator::GT, Operator::LT)
                  ]);
}
```

One filter per field, with the operators that field accepts. Only configured fields are resolved from the request and documented in OpenAPI, and a field name that collides with `order_by`, `offset`, `cursor`, `page` or `limit` is rejected when the route is built. See the [Filtering](/http/filtering) guide for more details.

## QUERY Requests

`enableQuery()` makes a filtered route additionally reachable as an [RFC 10008](https://datatracker.ietf.org/doc/rfc10008/) `QUERY` request, where the filters arrive as a JSON body instead of a query string:
...
The generator can opt back in via `forceIncludeExcludedRoutes()`, for an internal spec generated next to the public one. See [OpenAPI Generator](/documentation/openapi-generator).
