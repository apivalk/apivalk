...
* **Filtering**: A collection of `AbstractFilter` instances defining allowed filtering fields for the endpoint.
...
You can define which fields are allowed for filtering by using the `filtering()` method. This takes an array of `AbstractFilter` objects:
