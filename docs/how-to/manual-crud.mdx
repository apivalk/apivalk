...
final class CreatePetController extends AbstractApivalkController
{
    ...
    public function __invoke(CreatePetRequest $request): AbstractApivalkResponse
    {
...
The route is the only thing the controller declares. The request class is read from the `__invoke()` parameter and the documented responses from the `new` expressions its body returns, so `403` and `422` land in the spec without being listed here. Build a response anywhere but in `__invoke()` and it is [left out of the spec](/http/controller#documented-responses). See [Controller](/http/controller).
...
final class ViewPetController extends AbstractApivalkController
{
    ...
    public function __invoke(ViewPetRequest $request): AbstractApivalkResponse
    {
...
final class UpdatePetController extends AbstractApivalkController
{
    ...
    public function __invoke(UpdatePetRequest $request): AbstractApivalkResponse
    {
...
final class DeletePetController extends AbstractApivalkController
{
    ...
    public function __invoke(DeletePetRequest $request): AbstractApivalkResponse
    {
        $uuid = $request->path()->pet_uuid;
