...
The route is the only thing the controller declares. The request class is read from the `__invoke()` parameter and the documented responses from what its body returns, so `403` and `422` land in the spec without being listed here. See [Controller](/http/controller).
