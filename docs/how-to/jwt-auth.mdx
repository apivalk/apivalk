...
A configured `iss` or `aud` is mandatory: a token that omits the claim is rejected just like one that carries the wrong value. Pass an empty string to skip either check. Per RFC 7519 the `aud` claim is either a single string or a list of strings, and the token passes as soon as one entry equals the configured audience.

## 2. Register the security scheme

This name is what your routes reference via `RouteAuthorization`, at runtime and in the generated document. Register it before you construct `Apivalk`, otherwise the constructor rejects the routes pointing at it.
...
The OpenAPI generator reads the schemes from the same configuration — see [Generate OpenAPI and docblocks](/how-to/generate-openapi).

Add the audiences the scheme accepts as the last argument when tokens of a foreign audience must be rejected:

```php
$configuration->addSecurityScheme(
    SecuritySchemeObject::http('BearerAuth', 'bearer', 'JWT Bearer', 'JWT', 'https://api.example.com')
);
```
...
| `aud` (string or array) | `$identity->getAud()`, always a list |

A token that carries none of the permission claims gets an empty permission list, a token without `aud` an empty audience list.

A security scheme can require one of those audiences on top of scopes and permissions, see [Security Middleware](/middleware/security).

If `iss` or `aud` are missing or don't match the values you passed into the constructor, or the signature is invalid, or the token is expired, `authenticate()` returns `null` and the middleware falls back to `GuestAuthIdentity`. It never throws.
...
- **Always guest, even with a valid token.** Confirm the token carries `iss` and `aud`, that `iss` **exactly** matches what you passed into the constructor (trailing slash matters) and that `aud` contains the configured audience. Confirm the JWKS URL is reachable from your PHP container — `JwtAuthenticator::httpGet()` times out after 5 seconds.
