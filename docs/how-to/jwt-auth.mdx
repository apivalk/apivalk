...
A configured `iss` or `aud` is mandatory: a token that omits the claim is rejected just like one that carries the wrong value. Pass an empty string to skip either check.
...
A token that carries none of the permission claims gets an empty permission list.
...
- **Always guest, even with a valid token.** Confirm the token carries `iss` and `aud` and that both **exactly** match what you passed into the constructor (trailing slash on `iss` matters). Confirm the JWKS URL is reachable from your PHP container — `JwtAuthenticator::httpGet()` times out after 5 seconds.
