...
3. **Register a security scheme on the `ApivalkConfiguration`.** The `name` you give it (e.g. `BearerAuth`, `ApiKeyAuth`) becomes the link between your routes, the middleware and the OpenAPI spec.
...
`'BearerAuth'` must match the `name` of a `SecuritySchemeObject` registered with `$configuration->addSecurityScheme(...)`, otherwise `new Apivalk($configuration)` throws — see [OpenAPI Generator / Security Schemes](/documentation/openapi-generator#security-schemes-and-components).

### What SecurityMiddleware does with this

- A guest on a route with a `RouteAuthorization` → `401 Unauthorized`. Passing *no* requirements (just the scheme name) therefore means "any authenticated user".
- The scheme lists audiences and the identity carries none of them → `401 Unauthorized`.
- An authenticated identity missing a required scope or permission → `403 Forbidden`.
...
`AbstractAuthIdentity` already implements `getAud()` on top of its protected `$aud` property, which defaults to `[]`. Set `$this->aud` in your constructor when the credential is bound to audiences; leave it alone when the concept doesn't apply, but note that such an identity is rejected on every route whose scheme lists audiences. Never redeclare `$aud` as `private` in the subclass — that is a fatal error.
