...
// 2. The security schemes come from the configuration the bootstrap registered them on:
//    $configuration->addSecurityScheme(
//        SecuritySchemeObject::http('BearerAuth', 'bearer', 'JWT Bearer', 'JWT')
//    );
//    The name must match RouteAuthorization('BearerAuth', ...).
...
// 4. Generate
$generator = new OpenAPIGenerator($apivalk, $info, $servers);
...
docker compose run --rm php74 php bin/generate-openapi
```
...
```bash
docker compose run --rm php74 php bin/generate-docblocks && \
docker compose run --rm php74 php bin/generate-openapi
```
