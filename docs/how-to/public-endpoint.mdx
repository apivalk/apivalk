...
class GetHealthController extends AbstractApivalkController
{
    ...
    public function __invoke(GetHealthRequest $request): AbstractApivalkResponse
    {
...
- **OpenAPI coverage** → `GET /health` shows up in the generated spec with a `200` response whose schema matches `GetHealthResponse::getDocumentation()`, plus a `405`. No authorization, no rate limit and nothing to validate means no other error the route could produce, so none is documented.
...
- **Return 503 when a dependency is down** — construct a second response class (e.g. `ServiceUnavailableResponse`) with `new` in the failing branch of `__invoke()`. Both branches end up in the spec; a branch that delegates to a helper does not.
