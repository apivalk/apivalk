...
- **Return 503 when a dependency is down** — return a second response class (e.g. `ServiceUnavailableResponse`) from the failing branch of `__invoke()`. Both branches end up in the spec.
