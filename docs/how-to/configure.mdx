...
That's a complete, public-only API. Hitting a configured `GET /health` route goes through `Validate → Controller` and returns JSON.
...
- `RequestValidationMiddleware` runs as close to the controller as possible → add it last.
...
$stack->add(new SecurityMiddleware($configuration->getSecuritySchemes()));
