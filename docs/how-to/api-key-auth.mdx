...
## 5. Register the security scheme

Tell Apivalk that this API is secured via an `apiKey` in the `X-Api-Key` header. The name (`ApiKeyAuth` below) is what your routes reference, at runtime and in the generated document.
...
Register both schemes (`BearerAuth` and `ApiKeyAuth`) on the configuration. Different routes can require different schemes; the `SecuritySchemeObject.name` passed to `RouteAuthorization` picks which one the middleware enforces and OpenAPI displays.
