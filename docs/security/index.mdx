...
A route that additionally requires the token to be issued for a specific audience, passed as the fourth argument. It is matched against the identity's audience list, which for a JWT comes from the `aud` claim. A token issued for several audiences passes as soon as one of them matches, a token without a matching audience gets a `401 Unauthorized` even when it is otherwise valid.
...
// AUDIENCE-RESTRICTED: Additionally requires the token to be issued for this audience
