...
- **`getAud(): string[]`**: Returns the audiences this identity is valid for. A JWT carries them in the `aud` claim, which is a single string or a list of strings.
- **`isAudGranted(string $aud): bool`**: Helper method to check if a specific audience is present. Used by the [SecurityMiddleware](/middleware/security) for the audience a route requires.
