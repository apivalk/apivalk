...
- **Claim Validation**: Validates the `iss` (issuer), `aud` (audience), and `exp` (expiration) claims. A configured issuer or audience has to be present in the token and match; an empty string disables that check.
