...
**Native PHP 7.4+ / DDEV / Lando** — Docker is optional. Install via `composer install` with your local setup.
...
docker compose run --rm php74 composer phpstan
```
...
- Tests must be runnable with `vendor/bin/phpunit` and pass on PHP 7.4.
