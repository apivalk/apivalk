...
## 🛠️ Core Component Customization
...
Apivalk ships with `FilesystemCache`, which stores items on the local disk. For high-scale environments, you might want to store the routing data in memory.

By implementing `CacheInterface`, you can create a cache provider for:

- **Redis / Memcached**
- **Database**
- **Swoole / RoadRunner shared memory**

[Learn how to implement your own cache](/cache/filesystem-cache)
...
The `ApivalkControllerFactory` is responsible for instantiating controllers. By implementing `ApivalkControllerFactoryInterface`, you can customize this process—for example, to inject dependencies from your container into controller constructors.

## 🌐 HTTP Layer Customization
...
2. **Remove** built-in middlewares if they don't fit your use case.
